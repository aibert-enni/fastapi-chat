    ...
    async def update_image(
        ...
        if exist_file_path.exists() and exist_file_path.is_file():
            try:
                ...
                # Не критично - новый файл уже создан и БД обновлена

        # 1. Сначала создаем новый файл
        file_result, new_file_path = cls.add_file(file, folder_name, file_name)

        # 2. Проверяем, что файл создался успешно
        if not file_result:
            raise HTTPException(
        ...
        logger.info(f"New file saved to {new_file_path}")

        try:
            # 3. Обновляем базу данных
            old_path = db_file.file_path  # Сохраняем старый путь
            ...
            # 5. Если что-то не так с БД, удаляем новый файл
            try:
